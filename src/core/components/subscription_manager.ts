...
export default class {
  ...
  _processSubscribeResponse(status: StatusAnnouncement, payload: SubscribeEnvelope) {
    if (status.error) {
      ...
      if (status.category === categoryConstants.PNTimeoutCategory) {
        ...
        this._reconnectionManager.onReconnection(() => {
          ...
          let reconnectedAnnounce = new StatusAnnouncement();

          reconnectedAnnounce.category = categoryConstants.PNReconnectedCategory;
          reconnectedAnnounce.operation = status.operation;
          reconnectedAnnounce.lastTimetoken = this._lastTimetoken;
          reconnectedAnnounce.currentTimetoken = this._currentTimetoken;
    ...
    if (!this._subscriptionStatusAnnounced) {
      let connectedAnnounce = new StatusAnnouncement();
    ...
    if (requestMessageCountThreshold && messages.length >= requestMessageCountThreshold) {
      let countAnnouncement = new StatusAnnouncement();
    ...
    messages.forEach((message: any) => {
      ...
      if (utils.endsWith(message.channel, '-pnpres')) {
        let announce = new PresenceAnnouncement();
        ...
        let announce = new MessageAnnouncement();
        announce.channel = null;
        announce.subscription = null;
