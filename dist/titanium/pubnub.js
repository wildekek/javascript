	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    value: function decrypt(data, customCipherKey, options) {
	      ...
	      try {
	        ciphertext = _hmacSha2.default.enc.Base64.parse(data);
	        plaintext = _hmacSha2.default.AES.decrypt({ ciphertext: ciphertext }, cipherKey, { iv: iv, mode: mode }).toString(_hmacSha2.default.enc.Utf8);
	      } catch (e) {
	        console.log({ op: 'decrypt', e: e, plaintext: plaintext });
	        throw e;
	      }

	      console.log({ ciphertext: ciphertext, plaintext: plaintext });

	      try {
	        return JSON.parse(plaintext);
	      } catch (e) {
	        console.log({ op: 'parse', e: e, plaintext: plaintext });
	        throw e;
	      }
	    }
	  }]);
