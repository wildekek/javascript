...
function Fa(a,d){function b(){f+d>za()?(clearTimeout(c),c=setTimeout(b,d)):(f=za(),a())}var c,f=0;return b}function Ia(a,d){var b=[];G(a||[],function(a){d(a)&&b.push(a)});return b}function Ka(a,d){return a.replace(ua,function(a,c){return d[c]||a})}function Da(a){var d="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(a){var c=16*Math.random()|0;return("x"==a?c:c&3|8).toString(16)});a&&a(d);return d}
function La(a){return!!a&&"string"!==typeof a&&(Array.isArray&&Array.isArray(a)||"number"===typeof a.length)}function G(a,d){if(a&&d)if(La(a))for(var b=0,c=a.length;b<c;)d.call(a[b],a[b],b++);else for(b in a)a.hasOwnProperty&&a.hasOwnProperty(b)&&d.call(a[b],b,a[b])}function Ma(a,d){var b=[];G(a||[],function(a,f){b.push(d(a,f))});return b}function Qa(a){return encodeURIComponent(a).replace(/[!'()*~]/g,function(a){return"%"+a.charCodeAt(0).toString(16).toUpperCase()})}
function Ra(a,d){var b=[];G(a,function(a,f){d?0>a.search("-pnpres")&&f.g&&b.push(a):f.g&&b.push(a)});return b.sort()}function Sa(a,d){var b=[];G(a,function(a,f){d?0>a.search("-pnpres")&&f.g&&b.push(a):f.g&&b.push(a)});return b.sort()}function Ta(){setTimeout(function(){la||(la=1,G(ma,function(a){a()}))},1E3)}var Ua,Va=14,S=8,Wa=D;
...
function Za(a,d){var b=12<=Va?3:2,c=[],f=[],c=[],f=[],r=a.concat(d),q;c[0]=GibberishAES.C.D(r);f=c[0];for(q=1;q<b;q++)c[q]=GibberishAES.C.D(c[q-1].concat(r)),f=f.concat(c[q]);c=f.slice(0,4*S);f=f.slice(4*S,4*S+16);return{key:c,v:f}}
function $a(a,d,b){var d=ab(d),c=Math.ceil(a.length/16),f=[],r,q=[];for(r=0;r<c;r++){var l=f,t=r,y=a.slice(16*r,16*r+16),u=[],p=ea,p=ea;16>y.length&&(p=16-y.length,u=[p,p,p,p,p,p,p,p,p,p,p,p,p,p,p,p]);for(p=0;p<y.length;p++)u[p]=y[p];l[t]=u}0===a.length%16&&f.push([16,16,16,16,16,16,16,16,16,16,16,16,16,16,16,16]);for(r=0;r<f.length;r++)f[r]=0===r?bb(f[r],b):bb(f[r],q[r-1]),q[r]=cb(f[r],d);return q}
function hb(a,d,b,c){var d=ab(d),f=a.length/16,r=[],q,l=[],t="";for(q=0;q<f;q++)r.push(a.slice(16*q,16*(q+1)));for(q=r.length-1;0<=q;q--)l[q]=ib(r[q],d),l[q]=0===q?bb(l[q],b):bb(l[q],r[q-1]);for(q=0;q<f-1;q++)t+=Xa(l[q]);var t=t+Xa(l[q],v),y;if(c)y=t;else try{y=decodeURIComponent(escape(t))}catch(u){throw"Bad Key";}return y}function cb(a,d){Wa=D;var b=jb(a,d,0),c;for(c=1;c<Va+1;c++)b=kb(b),b=ub(b),c<Va&&(b=vb(b)),b=jb(b,d,c);return b}
function ib(a,d){Wa=v;var b=jb(a,d,Va),c;for(c=Va-1;-1<c;c--)b=ub(b),b=kb(b),b=jb(b,d,c),0<c&&(b=vb(b));return b}function kb(a){var d=Wa?wb:xb,b=[],c;for(c=0;16>c;c++)b[c]=d[a[c]];return b}function ub(a){var d=[],b=Wa?[0,13,10,7,4,1,14,11,8,5,2,15,12,9,6,3]:[0,5,10,15,4,9,14,3,8,13,2,7,12,1,6,11],c;for(c=0;16>c;c++)d[c]=a[b[c]];return d}
...
function ab(a){var d=[],b=[],c,f,r=[];for(c=0;c<S;c++)f=[a[4*c],a[4*c+1],a[4*c+2],a[4*c+3]],d[c]=f;for(c=S;c<4*(Va+1);c++){d[c]=[];for(a=0;4>a;a++)b[a]=d[c-1][a];if(0===c%S){a=b[0];f=ea;for(f=0;4>f;f++)b[f]=b[f+1];b[3]=a;b=Ib(b);b[0]^=Jb[c/S-1]}else 6<S&&4==c%S&&(b=Ib(b));for(a=0;4>a;a++)d[c][a]=d[c-S][a]^b[a]}for(c=0;c<Va+1;c++){r[c]=[];for(b=0;4>b;b++)r[c].push(d[4*c+b][0],d[4*c+b][1],d[4*c+b][2],d[4*c+b][3])}return r}function Ib(a){for(var d=0;4>d;d++)a[d]=xb[a[d]];return a}
...
Ua={size:function(a){switch(a){case 128:Va=10;S=4;break;case 192:Va=12;S=6;break;case 256:Va=14;S=8;break;default:throw"Invalid Key Size Specified:"+a;}},h2a:function(a){var d=[];a.replace(/(..)/g,function(a){d.push(parseInt(a,16))});return d},expandKey:ab,encryptBlock:cb,decryptBlock:ib,Decrypt:Wa,s2a:Ya,rawEncrypt:$a,rawDecrypt:hb,dec:function(a,d,b){var a=Ob.G(a),c=a.slice(8,16),c=Za(Ya(d,b),c),d=c.key,c=c.v,a=a.slice(16,a.length);return a=hb(a,d,c,b)},openSSLKey:Za,a2h:function(a){var d="",b;
...
d.addEventListener(a,f,D):d.attachEvent?d.attachEvent("on"+a,f):d["on"+a]=f})},Wb=function(){return Ub("head")[0]},Xb=function(a,d,b){if(b)a.setAttribute(d,b);else return a&&a.getAttribute&&a.getAttribute(d)},Yb=function(a,d){for(var b in d)if(d.hasOwnProperty(b))try{a.style[b]=d[b]+(0<"|width|height|top|left|".indexOf(b)&&"number"==typeof d[b]?"px":"")}catch(c){}},Zb=function(a){return document.createElement(a)},ec=function(){return $b||ac()?0:ya()},gc=function(a){function d(a,b,c){eb||(eb=1,T.onerror=
x,clearTimeout(fb),c&&b&&(c.response.data=b,c.response.status=b.status||200),a||!b||Ja(b,c),setTimeout(function(){a&&gb();var b=Sb(Ha),c=b&&b.parentNode;c&&c.removeChild(b)},fc))}if($b||ac()){a:{var b,c,f=0,r=0,q,l=a.timeout||2E4,t=a.e||F(),y=a.data||{},u=a.f||F(),p=a.mode||"GET",e={request:{},response:{}},g=setTimeout(function(){j(1,{message:"timeout"},e||{request:{},response:{}})},l),h=function(){if(!r){r=1;clearTimeout(g);try{c=JSON.parse(b.responseText)}catch(a){return b&&(e.response.body=b.responseText),
...
b.responseText,b.status){case 200:break;default:try{c=JSON.parse(b.responseText),j(1,c,e)}catch(a){return j(1,{status:b.status,I:x,message:b.responseText},e)}}};q=Ea(a.url,y);var Ga=e.request;Ga.method=p;Ga.url=q;b.open("GET",q,m);m&&(b.timeout=l);b.send()}catch(db){j(0,x,e);$b=0;a=gc(a);break a}a=j}return a}var T=Zb("script");q=a.a;var Ha=ya(),eb=0,fb=setTimeout(function(){d(1,{message:"timeout"})},a.timeout||2E4),gb=a.e||F(),l=a.data||{},P={request:{},response:{}},Ja=a.f||F();window[q]=function(a){d(0,
a,P)};a.l||(T[hc]=hc);T.onerror=function(){d(1,x,P)};T.src=Ea(a.url,l);Xb(T,"id",Ha);Wb().appendChild(T);return d},ic=function(){if(!("onLine"in navigator))return 1;try{return navigator.onLine}catch(a){return v}},ac=function(){if(!vc||!vc.get)return 0;var a={id:ac.id++,send:F(),abort:function(){a.id={}},open:function(d,b){ac[a.id]=a;vc.get(a.id,b)}};return a},hc="async",wc=navigator.userAgent,fc=1E3,$b=-1==wc.indexOf("MSIE 6");window.console||(window.console=window.console||{});console.log||(console.log=
...
d)&&0;Dc&&Cc(a,d);yc[a]=d}catch(b){yc[a]=d}}};var Fc={list:{},unbind:function(a){Fc.list[a]=[]},bind:function(a,d){(Fc.list[a]=Fc.list[a]||[]).push(d)},fire:function(a,d){G(Fc.list[a]||[],function(a){a(d)})}},Gc=Sb("pubnub")||0,Hc=function(a){function d(a,b){return E?!E||!E[0]?Aa(E):"http"+X+"://"+(E[b%E.length]||E[0]||"pubsub.pubnub.com"):Aa(a,b)}function b(){}function c(){}function f(a,b){function c(b){b&&(Eb=za()-(b/1E4+(za()-d)/2),a&&a(Eb))}var d=za();b&&c(b)||z.time(c)}function r(a,b){lb&&lb(a,
b);lb=x;clearTimeout(na);clearTimeout(oa)}function q(){Ic&&z.time(function(a){f(F(),a);a||r(1,{error:"Heartbeat failed to connect to Pubnub Servers.Please check your network settings."});oa&&clearTimeout(oa);oa=setTimeout(q,jc)})}function l(){Jc()||r(1,{error:"Offline. Please check your network settings. "});na&&clearTimeout(na);na=setTimeout(l,1E3)}function t(A,b,c,d){var b=A.callback||b,e=A.error||s,f=A.result||Y,j=A.status||U,g=a.op_params||{},l=O(),d=d||{};d.auth||(d.auth=A.auth_key||M);A=[I,
"v1","channel-registration","sub-key",w];A.push.apply(A,c);l&&(d.callback=l);N({a:l,data:L(d),f:function(a,A){!b&&u(a,A,g,f,j);b&&m(a,b,e)},e:function(a){!e&&y(a,http_data,g,j);e&&h(a,e)},url:A})}function y(a,b,c,d){b=g(b,c);b.data=p(a);b.category="error";m(b,d)}function u(a,b,c,d,e){b=g(b,c);b.data=p(a);m(b,d,e)}function p(a){var b={};if("object"==typeof a){if(a.error)return a.message&&(b.message=a.message),a.payload&&(b.payload=a.payload),b;if(a.payload)return a.payload}return a}function e(){return{origin:I.split("://")[1],
ssl:"s"==X?v:D,uuid:J,auth_key:M}}function g(a,b){if(a&&b)for(var c in b)b.hasOwnProperty(c)&&(a[c]=b[c]);return a}function h(a,b){if("object"==typeof a&&a.error){var c={};a.message&&(c.message=a.message);a.payload&&(c.payload=a.payload);b&&b(c)}else b&&b(a)}function m(a,b,c){if("object"==typeof a){if(a.error){b={};a.message&&(b.message=a.message);a.payload&&(b.payload=a.payload);c&&c(b);return}if(a.payload){a.next_page?b&&b(a.payload,a.next_page):b&&b(a.payload);return}}b&&b(a)}function j(a){var b=
0;G(Ra(C),function(c){if(c=C[c])b++,(a||F())(c)});return b}function Ga(a){var b=0;G(Sa(V),function(c){if(c=V[c])b++,(a||F())(c)})}function db(a){if(Kc){if(!ba.length)return}else{a&&(ba.z=0);if(ba.z||!ba.length)return;ba.z=1}N(ba.shift())}function T(a){clearTimeout(mb);!va||!Ra(C).length?nb=D:(nb=v,z.origin_heartbeat({callback:function(a){kc&&kc({timetoken:a,origin:ga,heartbeat_retry_number:Z});Z=1;mb=setTimeout(T,va)},error:function(){Na&&Na({origin:ga,heartbeat_retry_number:Z});!Na&&s&&s({origin:ga,
heartbeat_retry_number:Z});a||1===lc?(fb(),Z=1,mb=setTimeout(T,va)):(Z++,mb=Z<lc?setTimeout(T,mc):setTimeout(function(){T(1)},mc))}}))}function Ha(){if(E)if(clearTimeout(nc),!oc||!Ra(C).length)nb=D;else{pc=v;for(var a=0;a<pa%E.length;a++)eb(a);nc=setTimeout(Ha,oc)}}function eb(a){z.origin_heartbeat({origin:E[a],callback:function(){a<pa%E.length&&fb(a,"Optimal Check success for "+E[a])}})}function fb(a,b){var e=ga,f="undefined"!==typeof a?a:++pa;pa=f;I=d(E||ob,f);ga=d(E||ob,f);Na&&Na({message:"switching origin",
old_origin:e,new_origin:ga});r(1,{message:b||"Heartbeat Failed. Changing Origin",old_origin:e,new_origin:ga});j(function(a){a.h&&!a.j&&(a.j=1,a.u(a.name))});Z=1;c()}function gb(){!Fb&&P()}function P(){clearTimeout(pb);!W||500<=W||1>W||!Ra(C,v).length&&!Sa(V,v).length?Fb=D:(Fb=v,z.presence_heartbeat({callback:function(){pb=setTimeout(P,W)},error:function(a){s&&s("Presence Heartbeat unable to reach Pubnub servers."+JSON.stringify(a));pb=setTimeout(P,W)}}))}function Ja(a,b){return qb.decrypt(a,b||qa)||
qb.decrypt(a,qa)||a}function bc(a,b,c){var d=D;if("undefined"===typeof a)return b;if("number"===typeof a)d=0<=a?D:v;else{if("boolean"===typeof a)return a?3E4:0;d=v}return d?(c&&c("Presence Heartbeat value invalid. Valid range ( x > 5000 or x = 0). Current Value : "+(b||5E3)),b||5E3):a}function cc(a){var b="",c=[];G(a,function(a){c.push(a)});var d=c.sort(),e;for(e in d){var f=d[e],b=b+(f+"="+Qa(a[f]));e!=d.length-1&&(b+="&")}return b}function L(a){a||(a={});G(Q,function(b,c){b in a||(a[b]=c)});return a}
function B(a){return Hc(a)}function dc(a){function b(a,A){var c=(a&65535)+(A&65535);return(a>>16)+(A>>16)+(c>>16)<<16|c&65535}function c(a,b){return a>>>b|a<<32-b}var d;d=a.replace(/\r\n/g,"\n");for(var a="",e=0;e<d.length;e++){var f=d.charCodeAt(e);128>f?a+=String.fromCharCode(f):(127<f&&2048>f?a+=String.fromCharCode(f>>6|192):(a+=String.fromCharCode(f>>12|224),a+=String.fromCharCode(f>>6&63|128)),a+=String.fromCharCode(f&63|128))}e=a;d=[];for(f=0;f<8*e.length;f+=8)d[f>>5]|=(e.charCodeAt(f/8)&255)<<
...
0:-1==wc.indexOf("MSIE 6");var Oa=a.subscribe_key||"";a.uuid||xc.get(Oa+"uuid");var Lc=a.leave_on_unload||0;a.xdr=gc;a.db=xc;a.error=a.error||Tb;a._is_online=ic;a.jsonp_cb=ec;a.hmac_SHA256=Rb;Ua.size(256);var qc=Ua.s2a("0123456789012345");a.crypto_obj={encrypt:function(a,b){if(!b)return a;var c=Ua.s2a(dc(b).slice(0,32)),d=Ua.s2a(JSON.stringify(a)),c=Ua.rawEncrypt(d,c,qc);return Ua.Base64.encode(c)||a},decrypt:function(a,b){if(!b)return a;var c=Ua.s2a(dc(b).slice(0,32));try{var d=Ua.Base64.decode(a),
e=Ua.rawDecrypt(d,c,qc,D);return JSON.parse(e)}catch(f){}}};a.params={pnsdk:"PubNub-JS-Web/3.7.10"};var Mc=+a.windowing||10,Nc=+a.subscribe_timeout||31E4,ia=+a.non_subscribe_timeout||ia,jc=+a.keepalive||6E4,Ic=a.timecheck||0,rc=a.noleave||0,R=a.publish_key||"demo",w=a.subscribe_key||"demo",M=a.auth_key||"",sb=a.secret_key||"",sc=a.hmac_SHA256,X=a.ssl?"s":"",ob="http"+X+"://"+(a.origin||"pubsub.pubnub.com"),E=a.origins,ba=[],Gb=v,Eb=0,Hb=0,tc=0,lb=0,Ba=a.restore||0,ca=0,Pa=D,C={},V={},da={},pb=x,$=
bc(a.heartbeat||a.pnexpires||0,a.error),W=a.heartbeat_interval||$/2-1,Fb=D,mb=x,va=a.origin_heartbeat_interval||6E4,lc=a.origin_heartbeat_max_retries||2,mc=a.origin_heartbeat_interval_after_failure||1E4,nb=D,pc=D,nc=x,oc=a.optimal_origin_check_heartbeat_interval||15E3,Kc=a.no_wait_for_pending,uc=a["compatible_3.5"]||D,N=a.xdr,Q=a.params||{},s=a.error||F(),Jc=a._is_online||function(){return 1},O=a.jsonp_cb||function(){return 0},ja=a.db||{get:F(),set:F()},qa=a.cipher_key,Y=a.result||F(),U=a.status||
F(),kc=a.origin_heartbeat_callback,Na=a.origin_heartbeat_error_callback,J=a.uuid||!a.unique_uuid&&ja&&ja.get(w+"uuid")||"",ka=a.instance_id||D,aa="",na,oa;2===$&&(W=1);var qb=a.crypto_obj||{encrypt:function(a){return a},decrypt:function(a){return a}},pa=-1,Z=1,I=d(E||ob,++pa),ga=d(E||ob,pa),z={add_origin:function(a){E.push(a)},remove_origin:function(a){for(var b in E)E[b]===a&&(E[b]=D)},LEAVE:function(a,b,c,d,e){var c={uuid:J,auth:c||M},f=I,d=d||F(),j=e||F(),e=O();if(0<a.indexOf(ra))return v;if(uc&&
(!X||"0"==e)||rc)return D;"0"!=e&&(c.callback=e);ka&&(c.instanceid=aa);N({l:b||X,timeout:ia,a:e,data:L(c),f:function(a){m(a,d,j)},e:function(a){h(a,j)},url:[f,"v2","presence","sub_key",w,"channel",encodeURIComponent(a),"leave"]});return v},LEAVE_GROUP:function(a,b,c,d,e){var c={uuid:J,auth:c||M},f=I,d=d||F(),j=e||F(),e=O();if(0<a.indexOf(ra))return v;if(uc&&(!X||"0"==e)||rc)return D;"0"!=e&&(c.callback=e);a&&0<a.length&&(c["channel-group"]=a);ka&&(c.instanceid=aa);N({l:b||X,timeout:ia,a:e,data:L(c),
f:function(a){m(a,d,j)},e:function(a){h(a,j)},url:[f,"v2","presence","sub_key",w,"channel",encodeURIComponent(","),"leave"]});return v},set_resumed:function(a){Pa=a},get_cipher_key:function(){return qa},set_cipher_key:function(a){qa=a},raw_encrypt:function(a,b){return qb.encrypt(a,b||qa)||a},raw_decrypt:function(a,b){return Ja(a,b)},get_heartbeat:function(){return $},set_heartbeat:function(a,b){$=bc(a,$,s);W=b||$/2-1;2==$&&(W=1);c();P()},get_heartbeat_interval:function(){return W},set_heartbeat_interval:function(a){W=
a;P()},get_version:function(){return"3.7.10"},get_origin_heartbeat_interval:function(){return va},set_origin_heartbeat_interval:function(a){va=a;T()},get_sub_origin:function(){return ga},getGcmMessageObject:function(a){return{data:a}},getApnsMessageObject:function(a){var b={aps:{badge:1,alert:""}};for(k in a)k[b]=a[k];return b},newPnMessage:function(){var a={};gcm&&(a.pn_gcm=gcm);apns&&(a.pn_apns=apns);for(k in n)a[k]=n[k];return a},_add_param:function(a,b){Q[a]=b},channel_group:function(b,c){var d=
...
b.cloak,z.channel_group(b,c))},channel_group_list_namespaces:function(b,c){b.op_params=a.op_params||{operation:"channel_group_list_namespaces",connection:"non-sub",wasAutoRetried:v,config:e()};t(b,c,["namespace"])},channel_group_remove_namespace:function(b,c){b.op_params=a.op_params||{operation:"channel_group_remove_namespace",connection:"non-sub",wasAutoRetried:v,config:e()};t(b,c,["namespace",b.namespace,"remove"])},history:function(a,b){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,
j=a.count||a.limit||100,g=a.reverse||"false",l=a.auth_key||M,r=a.cipher_key,H=a.channel,q=a.channel_group,p=a.start,K=a.end,t=a.include_token,ha={},z=O(),C={operation:"history",connection:"non-sub",wasAutoRetried:v,config:e()};if(!H&&!q)return s("Missing Channel");if(!b&&!d)return s("Missing Callback");if(!w)return s("Missing Subscribe Key");ha.stringtoken="true";ha.count=j;ha.reverse=g;ha.auth=l;q&&(ha["channel-group"]=q,H||(H=","));z&&(ha.callback=z);p&&(ha.start=p);K&&(ha.end=K);t&&(ha.include_token=
"true");N({a:z,data:L(ha),f:function(a,e){if("object"==typeof a&&a.error)c({message:a.message,payload:a.payload});else{for(var A=a[0],h=[],j=0;j<A.length;j++){var g=Ja(A[j],r);try{h.push(JSON.parse(g))}catch(l){h.push(g)}}!b&&u([h,a[1],a[2]],e,C,d,f);b&&m([h,a[1],a[2]],b,c)}},e:function(a,b){!c&&y(a,b,C,f);c&&h(a,c)},url:[I,"v2","history","sub-key",w,"channel",encodeURIComponent(H)]})},replay:function(a,b){var b=a.callback||b,c=a.error,d=a.auth_key||M,e=a.source,f=a.destination,h=a.stop,j=a.start,
g=a.end,l=a.reverse,r=a.limit,q=O(),p={};if(!e)return s("Missing Source Channel");if(!f)return s("Missing Destination Channel");if(!R)return s("Missing Publish Key");if(!w)return s("Missing Subscribe Key");"0"!=q&&(p.callback=q);h&&(p.stop="all");l&&(p.reverse="true");j&&(p.start=j);g&&(p.end=g);r&&(p.count=r);p.auth=d;N({a:q,f:function(a){m(a,b,c)},e:function(){b([0,"Disconnected"])},url:[I,"v1","replay",R,w,e,f],data:L(p)})},auth:function(a){M=a;c()},time:function(a){var b=O(),c={uuid:J,auth:M};
ka&&(c.instanceid=aa);N({a:b,data:L(c),timeout:ia,url:[I,"time",b],f:function(b){a(b[0])},e:function(){a(0)}})},publish:function(a,b){var c={operation:"publish",connection:"non-sub",wasAutoRetried:v,config:e()},d=a.message;if(!d)return s("Missing Message");var b=a.callback||b,f=a.error,j=a.result||Y,g=a.status||U,l=a.channel||d.channel,r=a.auth_key||M,q=a.cipher_key,f=a.error||d.error,g=a.status||U,p=a.post||D,rb="store_in_history"in a?a.store_in_history:v,K=O(),t="push";a.prepend&&(t="unshift");
if(!l)return s("Missing Channel");if(!R)return s("Missing Publish Key");if(!w)return s("Missing Subscribe Key");d.getPubnubMessage&&(d=d.getPubnubMessage());d=JSON.stringify(qb.encrypt(d,q||qa)||d);d=[I,"publish",R,w,0,encodeURIComponent(l),K,encodeURIComponent(d)];Q={uuid:J,auth:r};rb||(Q.store="0");ka&&(Q.instanceid=aa);ba[t]({a:K,timeout:ia,url:d,data:L(Q),e:function(a,b){!f&&y(a,b,c,g);f&&h(a,f);db(1)},f:function(a,d){!b&&u(a,d,c,j,g);b&&m(a,b,f);db(1)},mode:p?"POST":"GET"});db()},unsubscribe:function(a,
b){var d=a.channel,e=a.channel_group,f=a.auth_key||M,b=a.callback||b,h=a.error;ca=0;d&&(d=Ma((d.join?d.join(","):""+d).split(","),function(a){if(C[a])return a+","+a+ra}).join(","),G(d.split(","),function(a){var c=v;a&&(C[a]=0,a in da&&delete da[a],la&&(c=z.LEAVE(a,0,f,b,h)),c||b({action:"leave"}))}));e&&(e=Ma((e.join?e.join(","):""+e).split(","),function(a){if(V[a])return a+","+a+ra}).join(","),G(e.split(","),function(a){var c=v;a&&(V[a]=0,a in da&&delete da[a],la&&(c=z.LEAVE_GROUP(a,0,f,b,h)),c||
b({action:"leave"}))}));c()},subscribe:function(a,d){function f(c,d){if(H){var e=d||{};e.q=q;e.n="error";H&&u(c,e,ia,H)}if(a.error||b)(e=a.error||b||F())&&h(c,e)}function m(a){a?setTimeout(c,1E3):setTimeout(function(){z.time(m)},1E3);g(a?1:0)}function g(a,b){j(function(c){var d=c[xa[c.i][a].callback];c.i=xa[c.i][a].state;d&&d(c.name,b)});Ga(function(c){var d=c[xa[c.i][a].callback];c.i=xa[c.i][a].state;d&&d(c.name,b)})}function l(){function a(b,c,d){"undefined"!==typeof b[c]&&(b[d]=b[c],delete b[c]);
return v}function b(a,d){if(!a||"object"==typeof a&&"error"in a&&a.error)return f(a.error,d),setTimeout(c,1E3);rb(a[1]);ca=!ca&&Ba&&ja.get(w)||a[1];g(1,d);if(Pa&&!Ba)ca=0,Pa=D,ja.set(w,0);else{B&&(ca=1E4,B=0);ja.set(w,a[1]);var e,h="",j="";3<a.length?(h=a[3],j=a[2]):h=2<a.length?a[2]:Ma(Ra(C),function(b){return Ma(Array(a[0].length).join(",").split(","),function(){return b})}).join(",");var m=h.split(","),A=j?j.split(","):[];e=function(){var a=m.shift()||tc,b=A.shift(),c={};b?(a&&(0<=a.indexOf("-pnpres")&&
0>b.indexOf("-pnpres"))&&(b+="-pnpres"),c=V[b]||C[b]||{callback:F()}):c=C[a];a=[c.a||Hb,a.split(ra)[0]];b&&a.push(b.split(ra)[0]);return a};var q=za()-Eb-+a[1]/1E4;G(a[0],function(b){var c=e(),b=Ja(b,C[c[1]]?C[c[1]].cipher_key:x);c[0]&&c[0](b,d,a,c[2]||c[1],q,c[1])})}setTimeout(l,Q)}function d(b,e){if(!b||"object"==typeof b&&"error"in b&&b.error)return f(b.error,e),setTimeout(c,1E3);rb(b.t.t);ca=!ca&&Ba&&ja.get(w)||b.t.t;j(function(a){a.h||(a.h=1,a.s(a.name,e))});Ga(function(a){a.h||(a.h=1,a.s(a.name,
e))});if(Pa&&!Ba)ca=0,Pa=D,ja.set(w,0);else{B&&(ca=1E4,B=0);ja.set(w,b.t.t);var h=b.m,m;for(m in h){var g=h[m],A=g.c,q=g.b;if(A=C[q]||V[q]||C[A])if(A=A.callback){var q=g.d,p=e,r=g,H=g.b||g.c,s=g.c;g.o&&a(g.o,"t","timetoken")&&a(g.o,"r","region_code");g.p&&a(g.p,"t","timetoken")&&a(g.p,"r","region_code");a(g,"i","issuing_client_id");a(g,"s","sequence_number");a(g,"o","origination_timetoken");a(g,"p","publish_timetoken");a(g,"k","subscribe_key");a(g,"c","channel");a(g,"b","subscription_match");a(g,
"r","replication_map");a(g,"ear","eat_after_reading");a(g,"d","payload");a(g,"u","user_metadata");a(g,"w","waypoint_list");A(q,p,r,H,s,g)}}}setTimeout(l,Q)}var e=O(),h=Ra(C).join(","),A=Sa(V).join(",");if(h||A){h||(h=",");r();var q=L({uuid:J,auth:y});A&&(q["channel-group"]=A);2<JSON.stringify(da).length&&(q.state=JSON.stringify(da));$&&(q.heartbeat=$);ka&&(q.instanceid=aa);gb();!nb&&T();!pc&&Ha();lb=N({timeout:W,a:e,e:function(a,b){a&&a.error&&a.service?(f(a,b),m(1)):z.time(function(c){!c&&f(a,b);
m(c)})},data:L(q),url:[ga,(P?"v2/":"")+"subscribe",w,encodeURIComponent(h),e,ca],f:P?d:b})}}var q=a.channel,p=a.channel_group,t=a.result||Y,H=a.status||U,y=a.auth_key||M,rb=a.idle||F(),K=a.presence||0,E=a.noheresync||0,B=a.backfill||0,I=a.timetoken||0,W=a.timeout||Nc,Q=a.windowing||Mc,R=a.state,P=a.v2,X=a.heartbeat||a.pnexpires,Z=a.heartbeat_interval,ba=a.restore||Ba,ia={operation:"subscribe",connection:"sub",wasAutoRetried:v,config:e()};if(a.callback)var na=a.callback,d=function(a,b,c,d,e,f,h){na&&
...
(X||0===X||Z||0===Z)&&z.set_heartbeat(X,Z);q&&G((q.join?q.join(","):""+q).split(","),function(b){var c=C[b]||{};C[tc=b]={name:b,i:wa.B,h:c.h,j:c.j,g:1,a:Hb=P?oa:d,cipher_key:a.cipher_key,s:pa,u:qa,F:va};R&&(da[b]=b in R?R[b]:R);K&&(z.subscribe({channel:b+ra,callback:K,restore:ba}),!c.g&&!E&&z.here_now({channel:b,data:L({uuid:J,auth:y}),callback:function(a){G("uuids"in a?a.uuids:[],function(c){K({action:"join",uuid:c,timestamp:Math.floor(za()/1E3),occupancy:a.occupancy||1},a,b)})}}))});p&&G((p.join?
p.join(","):""+p).split(","),function(b){var c=V[b]||{};V[b]={i:wa.B,name:b,h:c.h,j:c.j,g:1,a:Hb=P?oa:d,cipher_key:a.cipher_key,s:pa,u:qa,F:va};K&&(z.subscribe({channel_group:b+ra,callback:K,restore:ba,v2:P,auth_key:y}),!c.g&&!E&&z.here_now({channel_group:b,data:L({uuid:J,auth:y}),callback:function(a){G("uuids"in a?a.uuids:[],function(c){K({action:"join",uuid:c,timestamp:Math.floor(za()/1E3),occupancy:a.occupancy||1},a,b)})}}))});c=function(){r();setTimeout(l,Q)};if(!la)return ma.push(c);c()},here_now:function(a,
b){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,g=a.auth_key||M,j=a.channel,l=a.channel_group,q=O(),p=a.state,g={uuid:J,auth:g},r={operation:"here_now",connection:"non-sub",wasAutoRetried:D,config:e()};if(!("uuids"in a?a.uuids:1))g.disable_uuids=1;p&&(g.state=1);if(!b&&!d)return s("Missing Callback");if(!w)return s("Missing Subscribe Key");p=[I,"v2","presence","sub_key",w];j&&p.push("channel")&&p.push(encodeURIComponent(j));"0"!=q&&(g.callback=q);l&&(g["channel-group"]=l,!j&&p.push("channel")&&
p.push(","));ka&&(g.instanceid=aa);N({a:q,data:L(g),f:function(a,e){!b&&u(a,e,r,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,r,f);c&&h(a,c)},url:p})},where_now:function(a,b){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,g=a.auth_key||M,j=O(),l=a.uuid||J,g={auth:g},q={operation:"where_now",connection:"non-sub",wasAutoRetried:D,config:e()};if(!b&&!d)return s("Missing Callback");if(!w)return s("Missing Subscribe Key");"0"!=j&&(g.callback=j);ka&&(g.instanceid=aa);N({a:j,data:L(g),f:function(a,
e){!b&&u(a,e,q,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,q,f);c&&h(a,c)},url:[I,"v2","presence","sub_key",w,"uuid",encodeURIComponent(l)]})},state:function(a,b){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,g=a.auth_key||M,j=O(),l=a.state,q=a.uuid||J,p=a.channel,r=a.channel_group,g=L({auth:g}),t={operation:"state",connection:"non-sub",wasAutoRetried:D,config:e()};if(!w)return s("Missing Subscribe Key");if(!q)return s("Missing UUID");if(!p&&!r)return s("Missing Channel");"0"!=j&&(g.callback=
j);"undefined"!=typeof p&&C[p]&&C[p].g&&l&&(da[p]=l);"undefined"!=typeof r&&(V[r]&&V[r].g)&&(l&&(da[r]=l),g["channel-group"]=r,p||(p=","));g.state=JSON.stringify(l);ka&&(g.instanceid=aa);l=l?[I,"v2","presence","sub-key",w,"channel",p,"uuid",q,"data"]:[I,"v2","presence","sub-key",w,"channel",p,"uuid",encodeURIComponent(q)];N({a:j,data:L(g),f:function(a,e){!b&&u(a,e,t,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,t,f);c&&h(a,c)},url:l})},grant:function(a,b){var b=a.callback||b,c=a.error,d=a.result||Y,
f=a.status||U,g=a.channel||a.channels,j=a.channel_group,l=O(),q=a.ttl,p=a.read?"1":"0",r=a.write?"1":"0",t=a.manage?"1":"0",K=a.auth_key||a.auth_keys,z=a.op_params||{operation:"grant",connection:"non-sub",wasAutoRetried:D,config:e()};if(!b&&!d)return s("Missing Callback");if(!w)return s("Missing Subscribe Key");if(!R)return s("Missing Publish Key");if(!sb)return s("Missing Secret Key");var B=w+"\n"+R+"\ngrant\n",p={w:r,r:p,timestamp:Math.floor((new Date).getTime()/1E3)};a.manage&&(p.m=t);La(g)&&(g=
g.join(","));La(K)&&(K=K.join(","));"undefined"!=typeof g&&(g!=x&&0<g.length)&&(p.channel=g);"undefined"!=typeof j&&(j!=x&&0<j.length)&&(p["channel-group"]=j);"0"!=l&&(p.callback=l);if(q||0===q)p.ttl=q;K&&(p.auth=K);p=L(p);K||delete p.auth;B+=cc(p);g=sc(B,sb);g=g.replace(/\+/g,"-");g=g.replace(/\//g,"_");p.signature=g;N({a:l,data:p,f:function(a,e){!b&&u(a,e,z,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,z,f);c&&h(a,c)},url:[I,"v1","auth","grant","sub-key",w]})},mobile_gw_provision:function(a){var b=
a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,g=a.auth_key||M,j=O(),l=a.channel,p=a.op,q=a.gw_type,a=a.device_id,r={operation:"mobile_gw_provision",connection:"non-sub",wasAutoRetried:D,config:e()};if(!a)return s("Missing Device ID (device_id)");if(!q)return s("Missing GW Type (gw_type: gcm or apns)");if(!p)return s("Missing GW Operation (op: add or remove)");if(!l)return s("Missing gw destination Channel (channel)");if(!R)return s("Missing Publish Key");if(!w)return s("Missing Subscribe Key");
a=[I,"v1/push/sub-key",w,"devices",a];Q={uuid:J,auth:g,type:q};"add"==p?Q.add=l:"remove"==p&&(Q.remove=l);ka&&(data.instanceid=aa);N({a:j,data:Q,f:function(a,e){!b&&u(a,e,r,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,r,f);c&&h(a,c)},url:a})},audit:function(a,b){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,g=a.channel,j=a.channel_group,l=a.auth_key,p=O(),q=a.op_params||{operation:"audit",connection:"non-sub",wasAutoRetried:D,config:e()};if(!b&&!d)return s("Missing Callback");if(!w)return s("Missing Subscribe Key");
if(!R)return s("Missing Publish Key");if(!sb)return s("Missing Secret Key");var r=w+"\n"+R+"\naudit\n",t={timestamp:Math.floor((new Date).getTime()/1E3)};"0"!=p&&(t.callback=p);"undefined"!=typeof g&&(g!=x&&0<g.length)&&(t.channel=g);"undefined"!=typeof j&&(j!=x&&0<j.length)&&(t["channel-group"]=j);l&&(t.auth=l);t=L(t);l||delete t.auth;r+=cc(t);g=sc(r,sb);g=g.replace(/\+/g,"-");g=g.replace(/\//g,"_");t.signature=g;N({a:p,data:t,f:function(a,e){!b&&u(a,e,q,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,
b,q,f);c&&h(a,c)},url:[I,"v1","auth","audit","sub-key",w]})},revoke:function(a,b){a.read=D;a.write=D;a.op_params={operation:"revoke",connection:"non-sub",wasAutoRetried:D,config:e()};z.grant(a,b)},set_uuid:function(a){J=a;c()},get_uuid:function(){return J},isArray:function(a){return La(a)},get_subscibed_channels:function(){return Ra(C,v)},presence_heartbeat:function(a){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,a=O(),g={uuid:J,auth:M},j={operation:"presence_heartbeat",connection:"non-sub",
wasAutoRetried:D,config:e()};2<JSON.stringify(da).length&&(g.state=JSON.stringify(da));0<$&&320>$&&(g.heartbeat=$);"0"!=a&&(g.callback=a);var l;l=Ra(C,v).join(",");l=encodeURIComponent(l);var p=Sa(V,v).join(",");l||(l=",");p&&(g["channel-group"]=p);ka&&(g.instanceid=aa);N({a:a,data:L(g),timeout:ia,url:[I,"v2","presence","sub-key",w,"channel",l,"heartbeat"],f:function(a,e){!b&&u(a,e,j,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,j,f);c&&h(a,c)}})},stop_timers:function(){clearTimeout(na);clearTimeout(oa)},
origin_heartbeat:function(a){var b=a.callback||b,c=a.error,d=a.result||Y,f=a.status||U,g=O(),j={operation:"origin_heartbeat",connection:"non-sub",wasAutoRetried:D,config:e()};N({a:g,data:{uuid:J,auth:M},timeout:ia,url:[a.origin?"http"+X+"://"+a.origin:ga,"time","0"],f:function(a,e){!b&&u(a,e,j,d,f);b&&m(a,b,c)},e:function(a,b){!c&&y(a,b,j,f);c&&h(a,c)}})},xdr:N,ready:Ta,db:ja,uuid:Da,map:Ma,each:G,"each-channel":j,grep:Ia,offline:function(){r(1,{message:"Offline. Please check your network settings."})},
supplant:Ka,now:za,unique:ya,updater:Fa};J||(J=z.uuid());aa||(aa=z.uuid());ja.set(w+"uuid",J);na=setTimeout(l,1E3);oa=setTimeout(q,jc);pb=setTimeout(gb,W-3E3);f();var Oa=z,tb;for(tb in Oa)Oa.hasOwnProperty(tb)&&(B[tb]=Oa[tb]);B.css=Yb;B.$=Sb;B.create=Zb;B.bind=Vb;B.head=Wb;B.search=Ub;B.attr=Xb;B.events=Fc;B.init=B;B.secure=B;Vb("beforeunload",window,function(){if(Lc)B["each-channel"](function(a){B.LEAVE(a.name,0)});return v});if(a.notest)return B;Vb("offline",window,B.offline);Vb("offline",document,
B.offline);return B};Hc.init=Hc;Hc.secure=Hc;"complete"===document.readyState?setTimeout(Ta,0):Vb("load",window,function(){setTimeout(Ta,0)});var Oc=Gc||{};PUBNUB=Hc({notest:1,publish_key:Xb(Oc,"pub-key"),subscribe_key:Xb(Oc,"sub-key"),ssl:!document.location.href.indexOf("https")||"on"==Xb(Oc,"ssl"),origin:Xb(Oc,"origin"),uuid:Xb(Oc,"uuid")});window.jQuery&&(window.jQuery.PUBNUB=Hc);"undefined"!==typeof module&&(module.exports=PUBNUB)&&Ta();var vc=Sb("pubnubs")||0;if(Gc){Yb(Gc,{position:"absolute",
top:-fc});if("opera"in window||Xb(Gc,"flash"))Gc.innerHTML="<object id=pubnubs data=https://pubnub.a.ssl.fastly.net/pubnub.swf><param name=movie value=https://pubnub.a.ssl.fastly.net/pubnub.swf><param name=allowscriptaccess value=always></object>";PUBNUB.rdx=function(a,d){if(!d)return ac[a].onerror();ac[a].responseText=unescape(d);ac[a].onload()};ac.id=fc}}
